...
use anima_engine_lib::evolution::lineage::{
    FallbackLineageTracker, InMemoryLineageTracker, LineageTracker, RelationType,
};
...
fn test_in_memory_lineage_tracker() {
    ...
    root_gen.add_node(MorphologyNode {
        id: 0,
        length: 1.0,
        radius: 0.2,
        mass: 1.0,
    ...
    assert!(tracker
        .add_root(parent_2_id.clone(), root_gen.clone())
        .is_ok());
    ...
    assert!(nodes
        .iter()
        .any(|n| n.id == child_1_id && n.generation == 1));
    assert!(nodes
        .iter()
        .any(|n| n.id == parent_2_id && n.generation == 0));
    assert!(nodes
        .iter()
        .any(|n| n.id == child_2_id && n.generation == 2));

    // Verify relations
    assert_eq!(relations.len(), 3);
    assert!(relations.iter().any(|r| r.source_id == root_id
        && r.target_id == child_1_id
        && r.relation_type == RelationType::Clone));
    assert!(relations.iter().any(|r| r.source_id == child_1_id
        && r.target_id == child_2_id
        && r.relation_type == RelationType::Crossover));
    assert!(relations.iter().any(|r| r.source_id == parent_2_id
        && r.target_id == child_2_id
        && r.relation_type == RelationType::Crossover));
}
...
fn test_lineage_tier2_timeout_and_invalid_inputs() {
    ...
    assert!(
        !tracker.is_online(),
        "Should immediately mark as offline without panic"
    );
    ...
    assert!(tracker
        .add_root(root_id.clone(), empty_genotype.clone())
        .is_ok());
    ...
    assert!(tracker
        .add_root(current_parent.clone(), empty_genotype.clone())
        .is_ok());
...
fn test_lineage_tier3_crossover_multiple_parents() {
    ...
    for pid in parent_ids {
        assert!(relations.iter().any(|r| r.source_id == pid
...
fn test_lineage_tier4_evolutionary_workload() {
    ...
    assert!(relations.iter().any(|r| r.source_id == "gen-4-clone"
        && r.target_id == "gen-5-clone"
        && r.relation_type == RelationType::Clone));
    assert!(relations.iter().any(|r| r.source_id == "gen-4-mutate"
        && r.target_id == "gen-5-mutate"
        && r.relation_type == RelationType::Mutate));
    assert!(relations.iter().any(|r| r.source_id == "gen-4-clone"
        && r.target_id == "gen-5-crossover"
        && r.relation_type == RelationType::Crossover));
    assert!(relations.iter().any(|r| r.source_id == "gen-4-mutate"
        && r.target_id == "gen-5-crossover"
        && r.relation_type == RelationType::Crossover));
