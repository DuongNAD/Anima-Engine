...
use anima_engine_lib::core::ecs::{
    check_migration_boundaries_system, process_inbound_migrations_system, update_positions_system,
    Agent, AgentParentLineageIds, ChildrenLinks, InboundMigrationReceiver, MapBounds,
...
fn test_migration_bounds_too_narrow_panic() {
    ...
    schedule.add_systems((check_migration_boundaries_system, apply_deferred).chain());
...
async fn test_high_velocity_migration_loop_under_narrow_bounds() {
    ...
    println!(
        "After Tick 1 run, agents in world: {:?}",
        query
            .iter(&world)
            .map(|(p, v)| (p.0.x, v.0.x))
            .collect::<Vec<_>>()
    ...
    schedule.run(&mut world);
    println!(
        "After Tick 1 bounce-back spawn, agents in world: {:?}",
        query
            .iter(&world)
            .map(|(p, v)| (p.0.x, v.0.x))
            .collect::<Vec<_>>()
    ...
    schedule.run(&mut world);
    println!(
        "After Tick 2 run, agents in world: {:?}",
        query
            .iter(&world)
            .map(|(p, v)| (p.0.x, v.0.x))
            .collect::<Vec<_>>()
    ...
    schedule.run(&mut world);
    println!(
        "After Tick 2 bounce-back spawn, agents in world: {:?}",
        query
            .iter(&world)
            .map(|(p, v)| (p.0.x, v.0.x))
            .collect::<Vec<_>>()
    ...
    schedule.run(&mut world);
    println!(
        "After Tick 3 run, agents in world: {:?}",
        query
            .iter(&world)
            .map(|(p, v)| (p.0.x, v.0.x))
            .collect::<Vec<_>>()
    ...
    schedule.run(&mut world);
    println!(
        "After Tick 3 bounce-back spawn, agents in world: {:?}",
        query
            .iter(&world)
            .map(|(p, v)| (p.0.x, v.0.x))
            .collect::<Vec<_>>()
