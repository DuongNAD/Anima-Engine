...
use anima_engine_lib::core::ecs::{
    combat_system, Agent, AgentClass, Food, ParentAgent, Position, Predator, Prey, Rotation,
    Velocity,
};
...
fn test_predator_prey_classification() {
    ...
    // Query and check
    let predators: Vec<Entity> = world
        .query_filtered::<Entity, With<Predator>>()
        .iter(&world)
        .collect();
    let preys: Vec<Entity> = world
...
fn test_predator_tracking_inputs() {
    ...
    let pred_last = world
        .get::<anima_engine_lib::ai::hrrl::LastTransitionState>(predator)
        .unwrap();
    assert!((pred_last.state[0] - 0.0).abs() < 1e-4);
    assert!((pred_last.state[1] - 0.0).abs() < 1e-4);
    assert!(
        (pred_last.state[2] - 5.0).abs() < 1e-4,
        "Predator target should be nearest Prey at relative (0,0,5), got state[2]={}",
        pred_last.state[2]
    );

    // For Prey: should target nearest Food (dist vector (0, 0, 5) relative to prey at (0,0,5))
    let prey_last = world
        .get::<anima_engine_lib::ai::hrrl::LastTransitionState>(prey)
        .unwrap();
    assert!((prey_last.state[0] - 0.0).abs() < 1e-4);
    assert!((prey_last.state[1] - 0.0).abs() < 1e-4);
    assert!(
        (prey_last.state[2] - 5.0).abs() < 1e-4,
        "Prey target should be nearest Food at relative (0,0,5), got state[2]={}",
        prey_last.state[2]
    );
}
...
fn test_zero_allocation_combat_hot_path() {
    ...
    assert_eq!(
        allocs, 0,
        "Combat hot path should perform 0 heap allocations, but made {}",
        allocs
    );
}
