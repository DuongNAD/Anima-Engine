...
fn test_robust_mutation() {
    ...
    for _ in 0..200 {
        ...
        assert!(
            ...
            "Genotype became invalid after mutating from {:?}",
            prev_genotype
        );

        // Ensure bounds are respected
        assert!(
            genotype.nodes.len() >= 2,
            "Node count is less than 2: {}",
            genotype.nodes.len()
        );
        assert!(
            genotype.nodes.len() <= 15,
            "Node count is greater than 15: {}",
            genotype.nodes.len()
        );

        for node in &genotype.nodes {
            assert!(
                node.length >= 0.1 && node.length <= 5.0,
                "Node length out of bounds: {}",
                node.length
            );
            assert!(
                node.radius >= 0.05 && node.radius <= 1.0,
                "Node radius out of bounds: {}",
                node.radius
            );
            assert!(
                node.mass >= 0.05 && node.mass <= 10.0,
                "Node mass out of bounds: {}",
                node.mass
            );
        }
        ...
        for edge in &genotype.edges {
            ...
            let eps = 1e-4;
            assert!(
                edge.joint_anchor.x >= -radius - eps && edge.joint_anchor.x <= radius + eps,
                "Anchor x out of bounds: {} for parent radius {}",
                edge.joint_anchor.x,
                radius
            );
            assert!(
                edge.joint_anchor.y >= -radius - eps && edge.joint_anchor.y <= radius + eps,
                "Anchor y out of bounds: {} for parent radius {}",
                edge.joint_anchor.y,
                radius
            );
            assert!(
                edge.joint_anchor.z >= -length / 2.0 - eps
                    && edge.joint_anchor.z <= length / 2.0 + eps,
                "Anchor z out of bounds: {} for parent length {}",
                edge.joint_anchor.z,
                length
            );
            ...
            assert!(
                ...
                "Joint axis is not normalized: length is {}",
                axis_len
            );
        }
    }
...
fn test_subtree_crossover() {
    ...
    for _ in 0..50 {
        ...
        assert!(
            ...
            "Crossover result is invalid genotype: {:?}",
            child
        );
