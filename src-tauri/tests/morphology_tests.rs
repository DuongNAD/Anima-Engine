...
use anima_engine_lib::core::ecs::{
    init_world, metabolic_decay_system, MapBounds, ParentAgent, Position, Rotation, Segment,
    SegmentJointForce, Velocity,
};
use anima_engine_lib::evolution::genotype::{MorphologyEdge, MorphologyGenotype, MorphologyNode};
use anima_engine_lib::physics::dynamics::{
    integrate_physics_system, resolve_joints_system, JointConstraint, RigidBody,
};
...
fn decode_genotype_to_ecs(world: &mut World, genotype: &MorphologyGenotype) -> Entity {
    ...
    for edge in &genotype.edges {
        if let (Some(&src), Some(&tgt)) = (
            node_to_entity.get(&edge.source_node),
            node_to_entity.get(&edge.target_node),
        ) {
...
fn drive_joints_system(mut query: Query<(&CpgOscillator, &mut RigidBody, &mut SegmentJointForce)>) {
...
fn test_morphological_evolution_hot_path() {
    ...
    for (_entity, _parent, _body, vel, joint_force) in query_state.iter(&world) {
        ...
        assert!(
            vel.0 != Vec3::ZERO,
            "Physics did not integrate force into velocity / physical movement"
        );
        checked_forces = true;
    }
