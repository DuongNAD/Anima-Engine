...
use anima_engine_lib::core::ecs::{
    Agent, CognitiveState, InertiaComponent, ParentAgent, Position, Rotation, Segment,
    SensoryBufferComponent, Velocity,
};
...
fn test_decoupled_inference_cycle() {
    ...
    let req_batch = req_rx
        .try_recv()
        .expect("Expected a queued inference request in req_tx");
    ...
    assert_eq!(
        inertia_after_1.ticks_pending, 1,
        "Expected ticks_pending to increment to 1 on first pending frame"
    );

    // Record oscillator state
    let osc_after_1 = world.get::<CpgOscillator>(child_segment).unwrap().clone();
    assert!(
        osc_after_1.phase > 0.0,
        "Oscillator should have ticked once in the first run"
    );
    ...
    assert!(
        osc_after_2.phase > osc_after_1.phase,
        "CPG Oscillator did not tick when request was pending"
    );

    let inertia_after_2 = world.get::<InertiaComponent>(agent).unwrap();
    assert_eq!(
        inertia_after_2.ticks_pending, 2,
        "Expected ticks_pending to increment to 2"
    );
    ...
    let mut res_batch = recycle_res_rx
        .try_recv()
        .expect("Expected reusable response batch");
    ...
    assert!(
        (osc_res.frequency - 1.55).abs() < 1e-5,
        "Expected oscillator frequency to be 1.55, got {}",
        osc_res.frequency
    );
    assert!(
        (osc_res.amplitude - 1.2).abs() < 1e-5,
        "Expected oscillator amplitude to be 1.2, got {}",
        osc_res.amplitude
    );
    ...
    assert!(
        matches!(*final_state, CognitiveState::Ready),
        "Expected timeout to reset state to Ready, got {:?}",
        final_state
    );

    let final_inertia = world.get::<InertiaComponent>(agent).unwrap();
    assert_eq!(
        final_inertia.ticks_pending, 0,
        "Expected ticks_pending to reset to 0"
    );
    assert_eq!(
    ...
    assert!(
        (final_osc.frequency - 3.0).abs() < 1e-5,
        "Expected baseline oscillator frequency to be 3.0, got {}",
        final_osc.frequency
    );
    assert!(
        (final_osc.amplitude - 0.0).abs() < 1e-5,
        "Expected baseline oscillator amplitude to be 0.0, got {}",
        final_osc.amplitude
    );
}
