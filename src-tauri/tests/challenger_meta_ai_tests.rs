...
use anima_engine_lib::core::ecs::{
    apply_environmental_effects_system, combat_system, detect_food_collisions_system,
    metabolic_decay_system, spawn_food_system, ActiveEnvironmentEvent, Agent, CombatEvents, Food,
    FoodSpawnSettings, MapBounds, ParentAgent, Position, Predator, Prey, Segment, Velocity,
};
use anima_engine_lib::core::engine::{
    receive_environmental_events_system, EnvironmentalEventReceiver,
};
...
fn gemini_client_fallback_robustness() {
    ...
    assert_eq!(
        ...
        "Should fallback to Mock Client Epoch 1"
    );
...
fn environmental_systems_non_blocking_performance() {
    ...
    assert!(
        ...
        "Overall execution took too long: {:?}",
        total_elapsed
    );
...
fn ecs_hot_path_zero_allocations_i22() {
    ...
    assert_eq!(
        allocations, 0,
        "Expected exactly 0 heap allocations during the hot path tick loop, but got {}",
        allocations
    );
}
