...
| Licence texts | `npm run check:licenses` | up to date — **440 distributed, 266 distinct texts, 1 unresolved** | 0 |
| Vendored upstream licences | `npm run verify:upstream-licenses` (network, by hand) | **39 files, 24 commits, 19 repositories** match their pinned URLs byte for byte | 0 |
...
| Licence texts packaged | ~~not done~~ → **439 of 440 done; 1 blocked** | Superseded 2026-07-27, twice. `licensing/THIRD_PARTY_LICENSES.txt` reproduces the texts verbatim with a SHA-256 per file: 408 from the installed artifact, 31 vendored from the upstream repository at the immutable commit each release was published from (`licensing/upstream/`, read fail-closed, re-fetchable with `npm run verify:upstream-licenses`). The remaining 1 is `hexf-parse` 0.2.1, whose project has never published a licence file for that release; `licensing/UNRESOLVED.md` records the whole search. Engineering cannot generate a copyright holder's notice, so it stays open. Release-blocking for that component; the gate is `npm run check:licenses-complete`. |
