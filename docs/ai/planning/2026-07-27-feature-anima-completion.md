...
| 3b | The captures were not reproducible: only the day/night clock was pinned, while r3f's render clock, the frame delta, `WorldSky`'s stars and `WorldWeather`'s fog easing all still varied per load. Raised as items 13/14 of the addendum, after the row above was written | acceptance is exact byte identity — every view shot twice from two fresh browser contexts, no PNG written unless the two SHA-256s are equal (`canonical_views.spec.ts`); variance removed at each source rather than absorbed by a tolerance (`utils/sceneClock.ts`, `utils/captureMode.ts`), and the pixel-delta constants that had absorbed it are deleted | **closed** | `575c6dd` |
